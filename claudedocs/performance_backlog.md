...
### E-0. **[정정 2026-08-02] embed_cos 는 없애면 안 된다 — 대신 로컬 모델로 싸게 만든다**
...
**E-0**(e5-large 로컬 채택 — -46%, embed_cos·폴백 게이트 유지) → **E-2**(-68ms,
Session 재사용) → **E-5**(변별 셋 — 이후 "품질 동일" 주장을 검증할 수 있게) →
E-3(기동 워밍업: e5 모델 로드 4.4s) → E-1(게으른 계산, embed_cos 표시를 포기할지
결정한 뒤) → E-4 → D-14 → D-13 → 6

D-12 는 E-0 이 그 내용이다(철회 아님 — 모델 선택 기준이 "지연" 에서 "지연 + 게이트
분리력" 으로 바뀌었을 뿐이다).
