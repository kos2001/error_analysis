...
10-old. ~~`/recommend` 결과가 `/recommend/explain/stream`에서 통째로 재계산~~ —
    UI는 이슈 선택 시 `/recommend`(728ms), 이어서 "분석" 클릭 시 스트리밍 엔드포인트를
    호출하는데 후자가 `st["reco"].recommend(...)`를 같은 인자로 다시 실행한다
    (server.py:641). 사용자 1회 상호작용당 임베딩 API 2회 + rerank API 2회.
    → (query_rec, k, exclude_key, KB 버전) 키의 소용량 TTL 캐시.
    기대: 첫 토큰까지 **-730ms**, 외부 API 호출 절반.
...
**D-9**(게이트 무동작 — 신뢰성 직결, 1줄) → **D-10**(캐시, -730ms) → **D-11**(증분
임베딩 + 백그라운드 재빌드 + 워밍업) → **D-12**(로컬 임베딩 A/B, -400ms)
→ D-14(상설 계측) → D-13 → 6 → 15(보류)
